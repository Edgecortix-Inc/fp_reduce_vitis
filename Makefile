...
COMMON_FOLDER = $(ABS_COMMON_REPO)/common
XILINX_COMMON_FOLDER = $(COMMON_FOLDER)/xilinx
...
include $(XILINX_COMMON_FOLDER)/utils.mk

XSA := $(call device2xsa, $(DEVICE))
TEMP_DIR := ./temp.$(TARGET).$(XSA)
BUILD_DIR := ./build.$(TARGET).$(XSA)

VPP := v++
SDCARD := sd_card

#Include Libraries
include $(XILINX_COMMON_FOLDER)/opencl/opencl.mk
include $(XILINX_COMMON_FOLDER)/xcl2/xcl2.mk
CXXFLAGS += $(xcl2_CXXFLAGS)
LDFLAGS += $(xcl2_LDFLAGS)
HOST_SRCS += $(xcl2_SRCS)
CXXFLAGS += $(opencl_CXXFLAGS) -Wall -g -std=c++11
LDFLAGS += $(opencl_LDFLAGS)

NAME = fp_reduce
HOST_NAME = $(NAME)_host
KERNEL_NAME = $(NAME)_kernel

HOST_SRCS += host/$(HOST_NAME).cpp
...
CLFLAGS += -t $(TARGET) --platform $(DEVICE) --save-temps -j $(N_PROC)
ifneq ($(TARGET), hw)
	CLFLAGS += -g
endif

EXECUTABLE = $(HOST_NAME)
CMD_ARGS = --size 1600 --bin $(BUILD_DIR)/$(KERNEL_NAME).xclbin --verbose --verify
...
# Building kernel
$(TEMP_DIR)/$(KERNEL_NAME).xo: hls_kernel/$(KERNEL_NAME).cpp
	mkdir -p $(TEMP_DIR)
	$(VPP) $(CLFLAGS) --temp_dir $(TEMP_DIR) -c -k $(NAME) -I'$(<D)' -o'$@' '$<'
...
clean:
	...
	-$(RMDIR) hls_kernel/*.ll *v++* .Xil emconfig.json dltmp* xmltmp* *.log *.jou *.wcfg *.wdb

cleanall: clean
	-$(RMDIR) build.* sd_card*
	-$(RMDIR) temp.* *xclbin.run_summary qemu-memory-_* emulation/ _vimage/ pl* start_simulation.sh *.xclbin
